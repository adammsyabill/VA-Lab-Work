PwnTillDawn 10.150.150.12

First of all, we need to scan the entire subnetwork to get to the reachable IP address by using ping scan because it is lightweight.

    sudo namp -sn 10.150.150.10/24
![image alt](https://github.com/adammsyabill/VA-Lab-Work/blob/446baf15debd32cd09199b48af5b884acce942c8/image/Screenshot%202026-04-12%20200001.png)

Next, we will begin by enumerating the open ports on the target machine using a standard Nmap scan. At this stage, it’s useful to also perform service version detection and run common scripts to gather more detailed information about the target.

-sC (Default Scripts) Runs a set of default NSE (Nmap Scripting Engine) scripts Helps identify: Misconfigurations Basic vulnerabilities Service information

-sV (Service Version Detection) Attempts to determine: Service type (e.g., Apache, vsftpd) Version number

-Pn (No Ping) Skips host discovery (no ICMP ping) Treats the target as alive

     nmap -sC -sV -Pn 10.150.150.12

![image alt](https://github.com/adammsyabill/VA-Lab-Work/blob/279dd8dbdadfd62e2f46253908ac412cc043802a/image/Screenshot%202026-04-12%20200033.png)

During our nmap scan of the target, we discovered three open ports: 21 (FTP), 22 (SSH), and 16992 (Intel AMT). The FTP service is running vsFTPd version 2.0.8, which is quite old and even allows anonymous login with the username ftp. This is a potential security risk because outdated software often contains known vulnerabilities. For example, later versions such as vsFTPd 2.3.4 were found to have a backdoor vulnerability (CVE‑2011‑2523) that allowed remote code execution. Since 2.0.8 is also outdated, it is possible that similar exploits may apply here. The next step is to check Metasploit or other exploit databases to see if there are modules or scripts available that can be used against this version of vsFTPd, which could provide a way to gain access to the system.hing.

    msfconsole
    search vsftpd

![image alt](https://github.com/adammsyabill/VA-Lab-Work/blob/5c59e51da7ad06d54d93da7beb2bf10b9ceef79e/image/Screenshot%202026-04-12%20200053.png)

As expected, we get a hit on an exploit for the vsftpd 2.3.4 backdoor. Use it by selecting the exploit and setting the RHOST to poral (10.150.150.12) by using ‘show options’, and run the exploit.

    set RHOST 10.150.150.12

![image alt](https://github.com/adammsyabill/VA-Lab-Work/blob/0eb8f221822dfee1b98fd970cbce825b43f285ef/image/Screenshot%202026-04-12%20200201.png)

    id
    ls
    cat FLAG!.txt






  


